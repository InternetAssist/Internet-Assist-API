...
the JWT cookie genuinely cross-*site* for the alias.
...
| `CORS_ORIGINS` | yes | Comma-separated allowlist, e.g. `https://ia.uk,https://www.ia.uk,https://ia-webdesign.com,https://www.ia-webdesign.com,https://ticketing.ia.uk`. Must not contain `localhost` in production — `ProductionConfig.validate()` refuses to start otherwise. |
